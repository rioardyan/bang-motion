...
if len(sys.argv)>1:
    ...
    for t in ['<!doctype html>','<html lang="id">','<head>','</head>','<body>','</body>','</html>','<meta charset="utf-8">']:
        a=a.replace(t,'')
    a=a.replace('<title>Isi Legal Management System</title>','<title>Isi Legal Management System</title>')
    a=a.replace('html,body{width:100%;height:100%;background:#0B2426;overflow:hidden}',
                'html,body{width:100%;height:100%;background:#0B2426;overflow:hidden;color-scheme:dark}')
