...
if len(sys.argv)>1:
    ...
    for t in ['<!doctype html>','<html lang="id">','<head>','</head>','<body>','</body>','</html>','<meta charset="utf-8">']:
        a=a.replace(t,'')
    a=a.replace('<title>Demo Tool Legal Management</title>','<title>Demo Tool Legal Management</title>')
    a=a.replace('html,body{width:100%;height:100%;background:#DCE7E9;overflow:hidden}',
                'html,body{width:100%;height:100%;background:#DCE7E9;overflow:hidden;color-scheme:light}')
