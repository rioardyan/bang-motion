...
if len(sys.argv)>1:
    ...
    for t in ['<!doctype html>','<html lang="id">','<head>','</head>','<body>','</body>','</html>','<meta charset="utf-8">']:
        a=a.replace(t,'')
    a=a.replace('<title>Daftar Isi Legal Management</title>','<title>Daftar Isi Legal Management</title>')
    a=a.replace('html,body{width:100%;height:100%;background:#063E43;overflow:hidden}',
                'html,body{width:100%;height:100%;background:#063E43;overflow:hidden;color-scheme:dark}')
